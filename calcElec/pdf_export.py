"""Exportación profesional de PDF con manejo de errores"""
import logging
...
def export_history_pdf(entries, filepath):
    """Exporta historial a PDF con validación robusta
    
    Args:
        entries: Lista de diccionarios con cálculos
        filepath: Ruta de salida del archivo
        
    Returns:
        str: Ruta del archivo generado
        
    Raises:
        ValueError: Si entries está vacío
        IOError: Si hay error de escritura
    """
    try:
        # Validar entradas
        if not entries:
            logger.warning("Intento de exportar PDF sin datos")
            raise ValueError("No hay datos para exportar")
        
        logger.info(f"Generando PDF con {len(entries)} cálculos en {filepath}")
        ...
        logger.error(f"Error de validación al generar PDF: {ve}")
        raise
    except IOError as ioe:
        logger.error(f"Error de E/S al generar PDF: {ioe}")
        raise
    except Exception as e:
        logger.error(f"Error inesperado al generar PDF: {e}", exc_info=True)
        raise IOError(f"No se pudo generar el PDF: {e}")
